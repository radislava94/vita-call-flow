...
export interface LeadDataForModal {
  id: string;
  name: string;
  telephone: string;
  address: string | null;
  ...
  quantity?: number;
  price?: number;
  prediction_lead_items?: LeadItemLocal[];
...
export function CallPopup({ open, onClose, lead, contextType }: CallPopupProps) {
  ...
  // Form state (local, only saved on Save click)
  ...
  // Products
  const [items, setItems] = useState<LeadItemLocal[]>([]);
  ...
  useEffect(() => {
    ...
    // Clone items
    const leadItems = lead.prediction_lead_items || [];
    if (leadItems.length > 0) {
      setItems(leadItems.map(i => ({ ...i })));
    } else if (lead.product) {
      // Legacy single product → create synthetic item
      setItems([{
        id: '__legacy__',
        lead_id: lead.id,
        product_id: null,
        ...
        price_per_unit: lead.price || 0,
        total_price: calcRowTotal(lead.quantity || 1, lead.price || 0),
      }]);
    } else {
      setItems([]);
    }
    ...
    Promise.all([
      ...
      apiGetCallLogs(contextType, lead.id),
      apiGetProducts(),
    ])
  ...
  const personalizedScript = script
    ...
    .replace(/\[Order ID\]/g, lead?.id?.slice(0, 8) || '___');
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-3 border-b px-5 py-3 pr-10">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="font-semibold text-card-foreground text-sm truncate">Order Editor</h2>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
            ...
            {/* Follow-up date (only for call_again) */}
